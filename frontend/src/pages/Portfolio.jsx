            ...
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                activeCategory === category 
                  ? 'bg-gold text-dark' 
                  : 'bg-dark-secondary text-gray-300 hover:bg-white/10'
              }`}
            >
              {category}
            </button>
          ...
          <AnimatePresence>
            {filteredProjects.map((project) => (
              <motion.div
                ...
                className="group relative rounded-2xl overflow-hidden bg-dark-card border border-white/5 aspect-[4/3] cursor-pointer"
              >
                <div className="absolute inset-0 bg-dark/20 group-hover:bg-dark/80 transition-colors duration-500 z-10" />
                <img 
                  src={project.image} 
                  alt={project.title} 
                ...
                <div className="absolute inset-0 z-20 flex flex-col justify-end p-8 translate-y-8 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-500">
                  <span className="inline-block px-3 py-1 bg-gold/20 text-gold text-xs font-bold rounded-full w-fit mb-3">
                    {project.category}
                  </span>
                  <h3 className="text-2xl font-bold font-montserrat text-white mb-2">{project.title}</h3>
                  <p className="text-gray-300 text-sm mb-4">⭐ {project.metric}</p>
                  <Link to="/contact" className="text-gold font-bold text-sm flex items-center gap-2 hover:text-white transition-colors">
